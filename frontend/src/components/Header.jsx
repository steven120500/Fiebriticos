...
import { FaTimes, FaUser, FaBoxOpen, FaHistory, FaFutbol } from "react-icons/fa"; 
      ...
      <div className="flex items-center justify-between max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-3 sm:gap-5">
          <button 
            ...
            className="rounded-2xl bg-white/10 border border-white/20 p-1 w-12 h-12 flex items-center justify-center shadow-xl hover:border-fiebriVerde transition-all overflow-hidden group"
          >
            {user ? (
              <div className="bg-fiebriVerde w-full h-full flex items-center justify-center">
                <span className="font-black text-sm tracking-tighter text-fiebriAzul">
        ...
        {sidebarOpen && (
          <>
            <motion.div 
              ...
              animate={{ opacity: 1 }} 
              exit={{ opacity: 0 }} 
              className="fixed inset-0 z-[100] bg-fiebriAzul/60 backdrop-blur-md" 
            ...
            <motion.div 
              ...
              className="fixed top-0 right-0 h-full w-80 bg-white z-[101] shadow-2xl flex flex-col p-8" 
                ...
                <div className="mt-8 flex-grow">
                  <div className="mb-10 border-b border-fiebriGris pb-8 text-center">
                    <div className="w-20 h-20 bg-fiebriAzul rounded-3xl flex items-center justify-center mx-auto mb-4 shadow-xl border-b-4 border-fiebriVerde">
                      <span className="text-fiebriVerde font-black text-2xl">
                        {getInitials(user.firstName || user.username)}
                      </span>
                    </div>
                    <p className="text-fiebriAzul font-black text-2xl truncate">
                      {user.firstName || user.username}
                    </p>
                    <p className="text-gray-400 text-[9px] uppercase font-black tracking-widest mt-1">Miembro del Club</p>
                  </div>
        ...
        {showContacto && (
          <div className="fixed inset-0 bg-fiebriAzul/80 backdrop-blur-xl z-[9999] flex items-center justify-center p-4">
            <motion.div 
              ...
              className="bg-white p-8 rounded-[2.5rem] shadow-2xl relative w-full max-w-md border-b-8 border-fiebriVerde"
            >
              <button onClick={() => setShowContacto(false)} className="absolute top-6 right-6 text-gray-300 hover:text-fiebriAzul transition">
                <FaTimes size={28} />
              </button>
              <h2 className="text-3xl font-black text-fiebriAzul uppercase mb-6 tracking-tighter italic">Hablemos</h2>
              <Contacto />
            </motion.div>
          </div>
        )}
